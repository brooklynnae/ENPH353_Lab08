...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def find_line(self, img, w, h):
        ...
        ret, mask = cv2.threshold(gray, 200, 255, cv2.THRESH_BINARY_INV) #white track, black background
    ...
    def process_image(self, data):
        ...
        x, y = self.find_line(cv_image, w, h)
        if self.not_in_frame:
            self.timeout += 1
            if self.timeout >= 30:
                done = True
        else:
            partition = int(x/(w/10))
            state[partition] = 1
            self.timeout = 0
